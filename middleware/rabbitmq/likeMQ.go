...
func (l *LikeMQ) consumerLikeAdd(messages <-chan amqp.Delivery) {
	for d := range messages {
		...
		for i := 0; i < config.Attempts; i++ {
			...
			var likeData repository.Like
			//先查询是否有这条数据
			likeInfo, err := repository.GetLikeInfo(userId, videoId)
			//如果有问题，说明查询数据库失败，打印错误信息err:"get likeInfo failed"
			if err != nil {
				log.Printf(err.Error())
				flag = true //出现问题
			} else {
				if likeInfo == (repository.Like{}) { //没查到这条数据，则新建这条数据；
					...
					if err := repository.InsertLike(likeData); err != nil {
						log.Printf(err.Error())
						flag = true //出现问题
					}
				} else { //查到这条数据,更新即可;
					//如果有问题，说明插入数据库失败，打印错误信息err:"update data fail"
					if err := repository.UpdateLike(userId, videoId, config.IsLike); err != nil {
						log.Printf(err.Error())
						flag = true //出现问题
					}
				}
